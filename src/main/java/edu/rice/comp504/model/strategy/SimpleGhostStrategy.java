...
public class SimpleGhostStrategy implements IStrategy{
    ...
    public void ghostUpdate(Map map, Pacman pacman, Ghost ghost) {
        if (canSee(map.getMap(), ghost.getY(), ghost.getX(), pacman.getY(), pacman.getX())) {
            catchPacman = true;
        } else if (abs(ghost.getX() - pacman.getX()) + abs(ghost.getY() - pacman.getY()) >= 30) {
            // if the distance between ghost and pacman is larger than 30, ghost will not want to catch pacman
            catchPacman = false;
        }
        if (catchPacman) {
            String direction = "";
